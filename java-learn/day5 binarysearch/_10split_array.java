...
public class _10split_array {
    ...
    static int splitArray(int[] nums,int m){
        ...
        for(int j=0;j<nums.length;j++){
            ...
            end += nums[j];// sum of the array
        }
        if (m == 1) return (int) end;

        //binary search
        while(start<end){
            //try for middle as potentiall answer
            int mid = start + (end-start)/2;
            //calculate how manu pices you can divide this in with this max sum
            int sum=0;
            int pieces=1;
            for(int num:nums){
                //you can not add in the subb array make new sub array
                if(sum+num>mid){
                    sum=num;
                    pieces++;//  new sub array adding part
                }else{
                    sum+=num;
                }
            }
